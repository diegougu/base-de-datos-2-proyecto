...
void displayGraphics(sf::RenderWindow& window, sf::Font& font, const vector<int>& values) {
    vector<sf::RectangleShape> bars;
    for (size_t i = 0; i < values.size(); ++i) {
        sf::RectangleShape bar(sf::Vector2f(40, static_cast<float>(values[i])));
        bar.setFillColor(sf::Color::Blue);
        bar.setPosition(50 + i * 60, 500 - values[i]);
        bars.push_back(bar);
    }

    vector<sf::Text> labels;
    for (size_t i = 0; i < values.size(); ++i) {
        sf::Text label(to_string(values[i]), font, 15);
        label.setFillColor(sf::Color::Black);
        label.setPosition(50 + i * 60, 510);
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                if (findButton.getGlobalBounds().contains(mousePosF) && !isFindView) {
                    cout << "Botón FIND presionado" << endl;
                    findResults.clear();
                    ...
                    if (veri) {
                        int pos = 0;
                        do {
                            cout << "ingresa la posicion del elemento dentro del csv: " << endl;
                            cin >> pos;
